...
| 42 | [撕离活跃标签之前的标签后活跃标签漂移](tab_detach_active_index_shift.md) | （评审发现）`FenceHost.RemoveTab(index)` 只钳位越界、不处理 `index < _activeTabIndex`——拖拽撕离非活跃标签（`PreviewMouseLeftButtonDown` 记录索引但不激活）时后续索引整体前移，活跃标签跳到下一个。修复：`RemoveAt` 后 `index < _activeTabIndex` 则递减 | 已修复 | 2026-07-02 |
| 43 | [多选后右键"删除"只删除 1 个文件](multiselect_rightclick_delete_only_one.md) | fence/overlay 多选后右键删除只删被点中的 1 个。**真因**：Shell `IContextMenu` 构建时只传单个 PIDL（`ShellContextMenu.Show` 单文件签名 + `GetUIObjectOf(cidl=1)`），UI 保留多选高亮但菜单只知道 1 个文件；overlay 菜单返回后也只查被右键文件。修复：新增多文件重载走 `SHCreateShellItemArrayFromIDLists`+`IShellItemArray.BindToHandler(BHID_SFUIObject)`（选区可跨用户/公共桌面两目录，`GetUIObjectOf(cidl=N)` 不适用），两调用点传整个选区，overlay 事后存在性检查遍历全部路径 | 已修复 | 2026-07-02 |
| 44 | [桌面「查看/排序/刷新」联动完全失效](shellwindows_dispatch_member_not_found.md) | overlay 图标不随「查看→大图标」变大、排序/刷新无反应。**真因**：`IShellWindows` 声明为 `InterfaceIsIDispatch`，.NET RCW 对其走「按名字」`GetIDsOfNames` 晚绑定（`[DispId]` 不生效），explorer 解析 `"FindWindowSW"` 失败抛 `DISP_E_MEMBERNOTFOUND`，被 `TryReadViewState` 的 catch 吞成静默失效。修复：改 dual/vtable 早绑定（`InterfaceIsDual` + 8 个占位槽 + 完整签名，`ppdispOut` 为末位 out 参数）。**规则：COM 接口声明一律 vtable 早绑定** | 已修复 | 2026-07-02 |
| 45 | [Win+E 资源管理器被误判为桌面，fence/overlay 被误 hoist 到其上方](explorer_defview_mistaken_for_desktop.md) | Explorer 盖住 fence 中心后，fence/overlay 被抬到 `HWND_TOPMOST` 浮到其上、每 5 秒复现。**真因**：资源管理器（CabinetWClass）的文件视图子窗口类名与真桌面相同（`SHELLDLL_DefView`/`SysListView32`），`IsDesktopWindow` 只看自身/单层父类名 → sunk 自愈探测把"被 Explorer 合法遮挡"误判为"沉到壁纸下"→ 误 hoist。修复：改 `GetAncestor(GA_ROOT)` 顶层祖先 ∈ {Progman, WorkerW} 判定；顺带修复 Explorer 内双击误隐藏（QuickHide）、滚轮误翻页（PageSwitch）、拖拽误框选（Marquee）三个同族 bug | 已修复 | 2026-07-03 |
| 46 | [PowerToys 命令面板等 topmost 悬浮窗前台后 fence/overlay 消失](topmost_palette_foreground_sinks_fences.md) | 热键唤出命令面板（实测 `WS_EX_TOPMOST\|WS_EX_TOOLWINDOW`）约两秒后 fence/overlay 消失。**真因**：面板抢前台后防抖恢复对本已在 `HWND_BOTTOM` 的窗口重复下发 `HWND_BOTTOM`，此时可视顶层仍是桌面（准桌面态，bug 24 同族）→ 被 DWM 推到壁纸下；5 秒定时器在面板前台下反复重沉与 sunk 自愈 hoist 形成振荡。修复：`SendToBottom` 守卫"前台 topmost **且目标在普通带**"才跳过（仅消除冗余降级；topmost 带 fence 照常降级，否则面板占前台时 Win+D 恢复应用后 fence 卡顶——首版一刀切的回归）；`_isTopmost` 分支面板前台不打断 Win+D/Peek 置顶；定时器降级后补 50ms sunk 复检 | 已修复 | 2026-07-03 |
...
9. **判定 `WindowFromPoint` 命中是否"真桌面"，必须用 `GetAncestor(GA_ROOT)` 顶层祖先类名 ∈ {Progman, WorkerW}，不能看命中窗口自身或单层父类名**（bug 45）—— shell 窗口类名不专属于桌面：资源管理器（CabinetWClass）的文件视图子窗口同样是 `SHELLDLL_DefView`/`SysListView32`，其内部还有 `WorkerW` 类子窗口。按自身/单层父类名匹配会把"被 Explorer 合法遮挡"误判为"沉到壁纸下"→ 误 hoist 到 Explorer 之上。两个谓词的输入域不可混用：`IsDesktopWindow`/`IsDesktopAtPoint`（GA_ROOT 判定）服务于 point-hit；`IsDesktopOrTaskbarWindow`（祖先链宽匹配 + `#32768` owner 语义）仅限前台顶层窗口，禁止喂入 `WindowFromPoint` 结果
10. **"前台是普通窗口"≠"HWND_BOTTOM 安全"；topmost 悬浮窗前台只跳过对普通带目标的冗余降级**（bug 46）——安全的充分条件是"前台是真正遮挡桌面的普通带窗口"。三个已知准桌面态反例：①最小化窗口抢前台（bug 35，`IsIconic` 判）；②托盘菜单关闭过渡态（bug 24，路径级 topmost 兜底）；③热键唤出的 `WS_EX_TOPMOST` 悬浮面板前台（bug 46，`HasTopmostStyle` 判）。`SendToBottom` 守卫必须是**双条件**：前台 topmost（不需要让路）**且**目标不带 topmost 样式（无级可降，重复下发 HWND_BOTTOM 只剩壁纸压制副作用）。⚠️ 一刀切只判前台会把"topmost 带 fence 的必要降级"也吞掉——面板长期占前台时 Win+D 恢复应用后 fence 永远卡在应用之上（首版回归）；只判目标又会卡死拖拽 `HWND_TOP` 的降回。另：面板前台不打断 `_isTopmost`（Win+D/Peek）置顶，与托盘菜单同待遇（bug 1 原则）
