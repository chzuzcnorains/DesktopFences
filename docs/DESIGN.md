...
- **拖动边缘**：调整 Fence 大小
- **双击标题栏**：Rollup 折叠（只显示标题栏，高度缩小到 ~32px）
...
**设计目标**：已被分类到 Fence 中的桌面文件在桌面隐藏；停用规则时对应文件从 Fence 移除并恢复桌面显示；退出时还原所有文件。无需手动刷新桌面。

**方案：Per-file `FileAttributes.Hidden` + `SHChangeNotify`**

曾尝试 SysListView32 整体隐藏方案，但该方案是「全有或全无」——无法在规则停用时选择性恢复个别文件，因此回退到 per-file 方案。`DesktopIconManager` 保留用于旧版本升级恢复。

**实现**：
- `HideFileOnDesktop(filePath)` — 设置 `FileAttributes.Hidden` → 调用 `SHChangeNotify(SHCNE_UPDATEITEM)` 通知 Explorer 立即刷新
- `UnhideFileOnDesktop(filePath)` — 移除 Hidden 属性 → 调用 `SHChangeNotify(SHCNE_UPDATEITEM)` 立即刷新
- `HideAllFenceFiles()` — 批量隐藏 → `SHChangeNotify(SHCNE_UPDATEDIR)` 批量刷新 → 持久化到 `hidden_files.json`
- `RestoreHiddenFiles()` — 批量恢复 → 批量刷新 → 清空持久化列表

**Shell 通知**（解决需手动刷新桌面的问题）：
- 单文件：`ShellFileOperations.NotifyShellItemChanged(path)` → `SHChangeNotify(SHCNE_UPDATEITEM)`
- 批量后：`ShellFileOperations.NotifyShellDirectoryChanged(dir)` → `SHChangeNotify(SHCNE_UPDATEDIR)`
- P/Invoke 在 `NativeMethods.cs`，公开 API 在 `ShellFileOperations.cs`

**规则变更时重新分类**（`ReEvaluateClassifiedFiles`）：
- 规则保存后（`ShowRuleEditor` → `RulesSaved`）触发
- 遍历所有 Fence 中的文件，用当前启用规则重新匹配
- 不匹配或匹配到其他 Fence 的文件 → 从当前 Fence 移除 + `UnhideFileOnDesktop` 恢复桌面显示
- 批量通知 Explorer 刷新 + 持久化 + 自动保存布局

**崩溃恢复**：
- `_hiddenFiles` 通过 `hidden_files.json` 持久化到 `%APPDATA%\DesktopFences\`
- 启动时加载，若非空说明上次异常退出 → 恢复所有文件 Hidden 属性 → 通知 Explorer → 清空列表
- 兼容旧版 SysListView32 方案：检查 `.desktop_icons_hidden` 标记文件，如有则调用 `DesktopIconManager.ShowIcons()` 恢复

**隐藏时机**：
1. 自动整理新分类文件时 → `OrganizeDesktopOnceAsync` 中调用 `HideFileOnDesktop`
2. 桌面新增文件匹配规则时 → `OnDesktopFilesAdded` 中调用 `HideFileOnDesktop`
3. 启动加载已保存 Fence 后 → `HideAllFenceFiles` 批量隐藏
